...
//		routing.joinCheck();
    ...
    protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
        ...
        if (localInfo.equals(info)) {
            ...
            if (IRouting.USE_FINGER_TABLE) {
				 ...
				 */
                return findSuccessor(closestPrecedingFinger(id).addr, id);
            } else {
				/*
    ...
    /*
     * Find the successor of info
     */
    protected NodeInfo getSuccessor(NodeInfo info) throws Failed {
...
//		notifyContinue(db);
    ...
    public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
        ...
        int id = NodeKey(key);
        NodeInfo succ = this.findSuccessor(id);
        listenOn(succ.addr.toString(), key, listener);
    }

    public void listenOn(String uri, String key, EventListener listener) throws DHTBase.Failed {
        EventSource eventSource = client.listenForBindings(uri, info.id, key);
