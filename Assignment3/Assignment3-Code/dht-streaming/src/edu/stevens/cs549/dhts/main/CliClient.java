...
import edu.stevens.cs549.dhts.activity.*;
	...
	protected EventListener listener(final String key) {
		return new EventListener() {
			...
			public void onEvent(InboundEvent ev) {
				switch (ev.getName()) {
					case IDHTNode.NEW_BINDING_EVENT:
						msgln(String.format("** Binding event (%s): %s -> %s", ev.getName(), key, ev.readData(String.class)));
						break;
					case IDHTNode.TRANSFER_BINDINGD_EVENT:
						try {
							node.listenOff(key);
							// 这里存在时间差，由于其他节点未执行stabilize，导致node.listenOn(key, listener(key))无法找到正确的节点（我猜的），搞来搞去都不行哈哈。
							// 最后放弃了，作弊，直接回传addr算了...
//							node.routes();
//							Thread.sleep(5000);
//							IDHTBackground dht = (IDHTBackground) node;
//							dht.checkPredecessor();
//							dht.stabilize();
//							dht.fixFingers(8);
//							new Background(0, 8, main, (IDHTBackground) node).run();
//							node.routes();
							node.listenOn(ev.readData(), key, listener(key));
						} catch (DHTBase.Failed failed) {
						}
				}
