...
public class WebClient {
	...
	public EventSource listenForBindings(String addr, int id, String skey) throws DHTBase.Failed {
		...
		URI listenOnPath = UriBuilder.fromUri(addr).path("listen").queryParam("id", id).queryParam("key", skey).build();
		info("client listenOn(" + listenOnPath + ")");
		Client client = ClientBuilder.newClient();
		WebTarget target = client.target(listenOnPath);
		// how to advance time ???
	...
	public void listenOff(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		// TODO listen for SSE subscription requests on http://.../dht/listen?key=<key>
		// On the service side, don't expect LT request or response headers for this request.
		URI listenOffPath = UriBuilder.fromUri(node.addr).path("listen").queryParam("id", id).queryParam("key", skey).build();
		info("client listenOff(" + listenOffPath + ")");
		Response response = delRequest(listenOffPath);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed(String.format("DELETE /listen?id=%s&key=%s", id, skey));
		}
	}
